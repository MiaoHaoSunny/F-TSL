...
def load_network(dataset):
    ...
    """
    if dataset[0:6] == "tdrive":
        edge_path = "./data/" + dataset[0:6] + "/road/edge_weight.csv"
    elif dataset[0:4] == "rome":
        edge_path = "./data/" + dataset[0:4] + "/road/edge_weight.csv"
...
class DataLoader():
    def __init__(self,dataset):
        if dataset == "tdrive":
            ...
            self.config = yaml.safe_load(open('config_rome.yaml'))
        elif dataset[0:6] == "tdrive":
            self.config = get_taxi_config(dataset.split('_')[2])
            # config = yaml.safe_load(open('config_taxi_{}.yaml'.format()))
        elif dataset[0:4] == "rome":
            self.config = get_rome_taxi_config(dataset.split('_')[2])

        self.kseg = self.config["kseg"]
        self.train_set = 2000
        self.vali_set = 2800
        self.test_set = 6000
    ...
    def load(self, load_part):
        ...
        node_list_int = np.load(str(self.config["shuffle_node_file"]), allow_pickle=True)
        time_list_int = np.load(str(self.config["shuffle_time_file"]), allow_pickle=True)
        d2vec_list_int = np.load(str(self.config["shuffle_d2vec_file"]), allow_pickle=True)
    ...
    def ksegment_ST(self):
        ...
        kseg_coor_trajs = np.load(str(self.config["shuffle_kseg_file"]), allow_pickle=True)[:self.train_set]
        time_trajs = np.load(str(self.config["shuffle_time_file"]), allow_pickle=True)[:self.train_set]
    ...
    def get_triplets(self):
            ...
            for i in anchor_index:
                ...
                if str(self.config["distance_type"]) == "TP":
                    ...
                    if spatial_com.LCRS_dis(a_sample, p_sample) == spatial_com.longest_traj_len*2 or temporal_com.LCRS_dis(a_sample,p_sample) == temporal_com.longest_trajtime_len*2:
                        ok = False
                elif str(self.config["distance_type"]) == "NetERP":
        ...
        p = './data/{}/triplet/{}/'.format(str(self.config["dataset"]), str(self.config["distance_type"]))
        if not os.path.exists(p):
            os.makedirs(p)
        pickle.dump(apn_node_triplets,open(str(self.config["path_node_triplets"]),'wb'))
        pickle.dump(apn_time_triplets, open(str(self.config["path_time_triplets"]), 'wb'))
        pickle.dump(apn_d2vec_triplets, open(str(self.config["path_d2vec_triplets"]), 'wb'))

    def return_triplets_num(self):
        apn_node_triplets = pickle.load(open(str(self.config["path_node_triplets"]), 'rb'))
        return len(apn_node_triplets)

def triplet_groud_truth(dataset):
    if dataset == "tdrive":
    ...
    if str(config["dataset"]) == "tdrive" or str(config["dataset"])[0:6] == "tdrive":
    ...
    if str(config["dataset"]) == "rome" or str(config["dataset"])[0:4] == "rome":
        if str(config["distance_type"]) == "TP": coe = 8
...
def test_merge_st_dis(valiortest = None):
    s = np.load('./ground_truth/{}/{}/{}_spatial_distance.npy'.format(str(config["dataset"][0:4]), str(config["distance_type"]), valiortest))
...
def get_taxi_config(index):
    with open('config_taxi.yaml') as fp:
        yaml_str = fp.read()
        yaml_str = yaml_str.replace('tdrive_taxi_n', 'tdrive_taxi_{}'.format(index))
        return yaml.safe_load(yaml_str)

def get_rome_taxi_config(index):
    with open('config_rome_taxi.yaml') as fp:
        yaml_str = fp.read()
        yaml_str = yaml_str.replace('rome_taxi_n', 'rome_taxi_{}'.format(index))
        return yaml.safe_load(yaml_str)

if __name__ == "__main__":
    for i in range(1, 26):
        # config = get_taxi_config(i)
        config = get_rome_taxi_config(i)
