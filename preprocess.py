...
def read_graph(dataset):
    """
    Read network edges from text file and return networks object
    :param file: input dataset name
    :return: edage index with shape (n,2)
    """
    # dataPath = "./data/" + dataset
    # dataPath = "./data/tdrive"
    dataPath = "./data/rome"
    ...
    print("{0} road network has {1} edages.".format(config["dataset"], edge_index.shape[0]))
    print("{0} road network has {1} nodes.".format(config["dataset"], num_node))
...
if __name__ == "__main__":
    ...
    #         yaml_str = fp.read()
    #         yaml_str = yaml_str.replace('rome_taxi_n', 'rome_taxi_{}'.format(i))
    #
    #     with open('config_rome_taxi_tmp.yaml', 'w') as fp:
    #         fp.write(yaml_str)
    #
    #     config = yaml.safe_load(open('config_rome_taxi_tmp.yaml'.format(i)))
