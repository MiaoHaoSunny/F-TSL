...
def merge_Point_distance():
    ...
    np.save('./ground_truth/{}/Point_dis_matrix.npy'.format(dataset), res, allow_pickle=True)
...
def generate_node_edge_interation():
    ...
    edge = pd.read_csv('./data/{}/road/edge_weight.csv'.format(dataset[0:6]))
...
#         yaml_str = fp.read()
#         yaml_str = yaml_str.replace('tdrive_taxi_n', 'tdrive_taxi_{}'.format(i))
#
#     with open('config_taxi_tmp.yaml', 'w') as fp:
#         fp.write(yaml_str)

    # config = yaml.safe_load(open('config_taxi_tmp.yaml'.format(i)))
