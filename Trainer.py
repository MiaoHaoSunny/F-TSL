...
class STsim_Trainer(object):
    ...
    def ST_eval(self, load_model=None):
        ...
        if load_model != None:
            ...
            dataload = data_utils.DataLoader(self.dataset)
            ...
            with torch.no_grad():
                ...
                acc = test_method.test_model(embedding_vali,self.dataset, isvali=False)
                print(acc)

    def ST_train(self, load_model=None, load_optimizer=None, config=None):
        ...
        data_utils.triplet_groud_truth(self.dataset)

        optimizer = torch.optim.Adam([p for p in net.parameters() if p.requires_grad], lr=self.learning_rate,
                                     weight_decay=0.0001)
        lossfunction = LossFun(self.train_batch, self.distance_type,self.dataset)
            ...
            if epoch%50 == 0:
                ...
                with torch.no_grad():
                    ...
                    embedding_vali = test_method.compute_embedding(road_network=road_network, net=net,
                                                                   ...
                                                                   test_time=list(vali_d2vec_list),
                                                                   test_batch=self.test_batch)
                    acc = test_method.test_model(embedding_vali,"tdrive_TP", isvali=True)
                    s7 = time.time()
                    print("test time: ", s7-s6)
                    print('epoch:', epoch, 'HR10:', acc[0], 'HR50', acc[1],'HR1050:', acc[2], 'loss:', loss.item())
